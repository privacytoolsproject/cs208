...
meanRelease <- function(x, lower, upper, epsilon){
	...
	#lower <- min(x) # No, don't do this
	#upper <- max(x) # No, don't do this
...
regressionRelease <- function(y, x, ylower, yupper, xlower, xupper, epsilon){
	...
	n <- length(x)
	sens.Sxy <- ((xupper-xlower)*(yupper-ylower))   # Remove this
	sens.Sxx  <- ((xupper-xlower)^2)               # Remove this
	...
	release.Sxy <- sum((x - mean(x))*(y - mean(y)))  + rlap(mu=0, b=scale.Sxy, size=1)  # Remove this
	release.Sxx <- sum((x - mean(x))^2)              + rlap(mu=0, b=scale.Sxx, size=1)  # Remove this
...
for(i in 1:length(n.seq)){
	for(j in 1:length(ep.seq)){
		...
		for(k in 1:n.sims){
			...
			DPmean <- meanRelease(x=bootdata, lower=1, upper=16, epsilon=ep.seq[j])
			release <- DPmean$release
			sampleTrue <- DPmean$true

			## Regression release
			bootdata <- bootstrap(x=data.x, y=data.y, n=n.seq[i])
			DPregression <- regressionRelease(x=bootdata$x, y=bootdata$y, xlower=1, xupper=16, ylower=0, yupper=100000, epsilon=ep.seq[j])
...
dev.copy2pdf(file="./figs/laplaceReleases.pdf")
